...
export default function EventBrowser() {
  ...
  const sendSearchKey = (key) => { // the callback. Use a better name
    setSearchKey(key);
  }
  const sendFilteredEvents = (events) => { // the callback. Use a better name
...
function EventSearchBar({sendSearchKey}) {
  ...
  useEffect(() => {sendSearchKey(searchKey)}, [searchKey]);
        ...
        <div className="row">
          <div className="col-md-12 .col-event-search-bar">
            <div className="input-group event-search-bar shadow-sm">
              <TextField placeholder="Search" type="search" variant="standard" fullWidth size="small" value={searchKey}
...
function EventFilter({eventList, sendFilteredEvents}) {
  ...
  useEffect(() => {eventFilterHandler()}, [filterCategory, filterTime, filterPrivacy, eventList]);
  ...
  function eventFilterHandler() {
    filteredEvents = eventList.filter((event) => {
      ...
      if (filterTime != null && filterTime != '') {          
          ...
          if (time.getHours() >= 6 && time.getHours() < 12 && filterTime != 'morning')
            return false;
          else if (time.getHours() >= 12 && time.getHours() < 18 && filterTime != 'afternoon')
            return false;
          else if (time.getHours() >= 18 && time.getHours() < 21 && filterTime != 'evening')
            return false;
          else if (time.getHours() >= 21 && time.getHours() < 24 && filterTime != 'night')
            return false;
          else if (time.getHours() >= 0 && time.getHours() < 6 && filterTime != 'midnight')
            return false;
      }
    ...
    <div className="div-filter shadow">
      ...
      <div>
        <FormControl sx={{ m: 1, minWidth: 240 }}>
          ...
          <Select labelId="filter-event-category-label" id="filter-event-category" value={filterCategory} label="Event Category"
        ...
        <FormControl sx={{ m: 1, minWidth: 240 }}>
          ...
          <Select labelId="filter-event-time-label" id="filter-event-time" value={filterTime} label="Event Time"
        ...
        <FormControl sx={{ m: 1, minWidth: 240 }}>
          ...
          <Select labelId="filter-event-location-label" id="filter-event-location" value={filterLocation} label="Event Location"
        ...
        <FormControl sx={{ m: 1, minWidth: 240 }}>
          ...
          <Select labelId="filter-event-privacy-label" id="filter-event-privacy" value={filterPrivacy} label="Event Privacy"
...
function EventCardDeck(props) {  
  ...
  useEffect(() => {setFilteredEventList(props.filteredEvents);setDisplayedEventList(props.filteredEvents);}, [props.filteredEvents]);
  useEffect(() => {eventFilterHandler()}, [props.searchKey]);
  function eventFilterHandler() {
    if (props.searchKey != null && props.searchKey != '') {
      setDisplayedEventList(filteredEventList.filter((event) => {return event.name.toLowerCase().includes(props.searchKey.toLowerCase())}));
    }
    if (props.searchKey == null || props.searchKey == '') {
